// Tasks.tsx — /projects/[project_id]/tasks
  ...
  return (
    <tr className="group border-b border-white/[0.04] transition-colors hover:bg-white/[0.02]">
      ...
      <td className="px-5 py-3.5">
        <a href={`/projects/1/tasks/${task.id}`} className="block">
...
export default function Tasks() {
  const currentRole: UserRole = "team_lead"; // replace with sessionStorage role
  const canEdit = currentRole === "team_lead" || currentRole === "admin";
  const activeFilter: TaskStatus | "all" | "overdue" = "all";

  const overdueCnt = MOCK_TASKS.filter((t) => t.isOverdue).length;
  const counts = {
    ...
    <div className="min-h-screen bg-[#0b0f1a] text-white">
      ...
      <main className="mx-auto max-w-5xl px-7 py-10">
        ...
        <div className="mb-5 flex items-center gap-1.5 text-[12px] text-slate-500">
          ...
          <span>›</span>
          <a href="/projects/1" className="text-blue-400 hover:text-blue-300">Ethara Platform v2</a>
          <span>›</span>
        ...
        <div className="mb-6 flex items-end justify-between">
          <div>
            ...
            <p className="mt-1 text-[13px] text-slate-500">
              Ethara Platform v2 · {counts.all} tasks total
        ...
        <div className="mb-5 grid grid-cols-3 gap-3">
          {[
            { label: "Done",        value: counts.done,        cls: "border-emerald-500/20 bg-emerald-500/[0.07] text-emerald-300" },
            { label: "In Progress", value: counts.in_progress, cls: "border-blue-500/20 bg-blue-500/[0.07] text-blue-300" },
            { label: "To Do",       value: counts.todo,        cls: "border-slate-600/20 bg-slate-700/20 text-slate-400" },
          ].map((s) => (
